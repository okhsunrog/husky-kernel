"""Pinned husky builds; run with uv run scripts/forge.py."""
...
@contextmanager
def build_lock(work):
    """Prevent separate CLI runs from resetting sources during a build."""
    with (work / ".husky.lock").open("a") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError as error:
            raise RuntimeError(f"Another operation is using {work}") from error
        try:
            yield
        finally:
...
class Forge:
    ...
    def recipe_identity(self):
        inputs = [ROOT / name for name in ["versions.env", "pyproject.toml", "uv.lock"]]
    ...
    def logged(self, label, *args, cwd=None):
        ...
        self.report["steps"].append(
            {
                ...
                "adjustments": [
                    s for s in result.stdout.splitlines() if re.search(r"\b(offset|fuzz)\b", s)
                ],
            }
        )
        ...
        require(
            result.returncode == 0,
            f"{label} failed; see {self.work / 'patch-report.json'}",
        )

    def transformed(self, label, action):
        step = {"name": label, "exit_code": 0}
        try:
            step["files"] = action()
            print(f"{label}: {len(step['files'])} file(s) changed", flush=True)
            ...
            write(
                self.work / "patch-report.json",
                json.dumps(self.report, indent=2) + "\n",
            )

    def integrate_vpnhide(self):
        """Use the pinned Python integrator and preserve its full review bundle."""
        kmi = re.match(r"android\d+-\d+\.\d+", self.c["GKI_BRANCH"])[0]
        integrator = self.work / "vpnhide/builtin/scripts/integrate.py"
        require(
            integrator.is_file(),
            "Pinned vpnhide lacks the Python integrator; sync its new pin",
        )
    ...
    def local_patches(self):
        ...
        for line in series.read_text().splitlines():
            ...
            require(
                path.is_relative_to(series.parent.resolve()) and path.is_file(),
            ...
            for added_name in re.findall(
                r"^--- /dev/null\n\+\+\+ b/([^\t\n]+)", path.read_text(), re.M
            ):
                require(
                    not Path(added_name).is_absolute() and ".." not in Path(added_name).parts,
                    "Unsafe added patch path",
                )
                require(
                    not git(self.common, "ls-files", "--", added_name),
                    "Added patch file is already tracked",
                )
                require(
                    not (self.common / added_name).exists(),
                    "Added patch file already exists: " + added_name,
                )
    ...
    def doctor(self):
        ...
        check(
            ...
            lambda: require(
                ...
                in run("rustup", "target", "list", "--installed", capture=True),
                "install aarch64-linux-android",
            ),
        )
        ...
        for path in paths:
            check(
                ...
                lambda path=path: require(
                    path.is_file() and os.access(path, os.X_OK), "missing executable"
                ),
            )
    ...
    def owned(self):
        ...
        require(
            marker.exists() and json.loads(marker.read_text())["work"] == str(self.work),
    ...
    def adopt(self):
        ...
        write(
            self.work / ".husky-managed.json",
            json.dumps({"work": str(self.work), "backup": str(backup)}),
        )
    ...
    def component(self, path, url, rev, branch=None):
        ...
        require(
            not git(path, "status", "--porcelain", "--untracked-files=no"),
    ...
    def sync_components(self):
        ...
        self.component(
            self.ksu,
            self.c["KSU_NEXT_REPO"],
            self.c["KSU_NEXT_REV"],
            self.c["KSU_NEXT_REF"],
        )
        for name, prefix, branch in [
        ...
        for key, value in self.c.items():
            if key.endswith("_REV"):
                require(
                    ...
                    f"{key} must be a full commit SHA",
                )
        for p in projects:
            require(
                git(self.work / p.attrib["path"], "rev-parse", "HEAD") == p.attrib["revision"],
                f"AOSP revision mismatch: {p.attrib['path']}",
            )
        for path, key in [
            (self.ksu, "KSU_NEXT_REV"),
            ...
            require(
                ...
                f"Revision mismatch: {path}",
            )
            if path != self.ksu:
                require(
                    not git(path, "status", "--porcelain", "--untracked-files=normal"),
    ...
    def prepare(self):
        ...
        if inventory.exists():
            for name in json.loads(inventory.read_text()):
                ...
                require(
                    path.resolve().is_relative_to(self.common)
                    and not git(self.common, "ls-files", "--", name),
        ...
        self.transformed("susfs prepare", lambda: vendor_headers(self.common, "prepare"))
        self.logged(
            "susfs patch",
            "patch",
            "--batch",
            ...
            "--no-backup-if-mismatch",
            "-i",
            self.work / "susfs4ksu/kernel_patches/50_add_susfs_in_gki-android14-6.1.patch",
            cwd=self.common,
        )
        self.transformed("susfs restore", lambda: vendor_headers(self.common, "restore"))
        self.transformed("susfs compatibility", lambda: susfs_compatibility(self.common))
        self.logged(
            "zeromount patch",
            "patch",
            "--batch",
            ...
            "--no-backup-if-mismatch",
            "-i",
            ROOT / "patches/zeromount/60_zeromount-android14-6.1.patch",
            cwd=self.common,
        )
        ...
        for name in ["GET_APP_PROFILE", "SET_APP_PROFILE"]:
            require(
                re.search(
                    r'\.name = "' + name + r'"[^}]+\.perm_check = manager_or_root',
                    dispatch,
                ),
                "KSU lacks root profile permission",
            )
        self.configure()
    ...
    def configure(self):
        ...
        write(
            self.common / "arch/arm64/configs/gki_defconfig",
        ...
        write(
            self.common / "build.config.gki",
            re.sub(r"\bcheck_defconfig\b", "", original) + "\nexport TMPDIR=/tmp\n",
        )
        kleaf = self.work / "build/kernel"
        original = git(kleaf, "show", "HEAD:kleaf/impl/stamp.bzl")
        require("echo '-maybe-dirty'" in original, "Kleaf stamp implementation changed")
        write(
            kleaf / "kleaf/impl/stamp.bzl",
            original.replace("echo '-maybe-dirty'", "echo '" + self.c["STOCK_SCMVERSION"] + "'")
            + "\n",
        )
    ...
    def prepared(self):
        ...
        state = self.work / ".husky-prepared.json"
        require(
            state.exists() and json.loads(state.read_text()) == self.c,
            "Pins changed: run prepare",
        )
        recipe = self.work / ".husky-recipe"
        require(
            recipe.exists() and recipe.read_text() == self.recipe_identity(),
            "Recipe changed: run prepare",
        )
    ...
    def kernel(self):
        ...
        run(
            self.work / "tools/bazel",
            "build",
            "--config=fast",
            "--lto=" + self.c["LTO_MODE"],
            "--action_env=TMPDIR=/tmp",
            "--disk_cache=" + str(self.work / ".bazel-cache"),
            "//common:kernel_aarch64",
            cwd=self.work,
            env={**os.environ, "TMPDIR": "/tmp"},
        )
        require(
            (self.work / "bazel-bin/common/kernel_aarch64/Image").exists(),
            "No kernel Image produced",
        )
        self.verify_kernel()
        require(identity == self.kernel_identity(), "Kernel inputs changed during build")
        write(
            self.work / ".husky-kernel.json",
            json.dumps(
                {
                    "identity": identity,
                    "image": sha(self.work / "bazel-bin/common/kernel_aarch64/Image"),
                }
            ),
        )
    ...
    def kernel_identity(self):
        ...
        integrated = [
            self.common / "fs/zeromount.c",
            self.common / "include/linux/zeromount.h",
            self.common / "include/linux/vpnhide.h",
            self.common / "drivers/kernelsu",
        ]
        integrated.extend(p for p in (self.common / "security/vpnhide").rglob("*") if p.is_file())
        inventory = self.work / ".husky-local-files.json"
        if inventory.exists():
            integrated.extend(self.common / name for name in json.loads(inventory.read_text()))
        for folder in ["fs", "include/linux"]:
            integrated.extend(
                self.common / folder / p.name
                for p in (self.work / "susfs4ksu/kernel_patches" / folder).iterdir()
            )
        sources["integrated"] = {
            str(p.relative_to(self.common)): file_identity(p) for p in integrated
        }
    ...
    def builtin(self):
        ...
        run(
            ...
            "builtin/build.py",
            "--out",
            self.work / "vpnhide-builtin.zip",
            cwd=self.work / "vpnhide",
            env=env,
        )
    ...
    def package(self):
        ...
        require(
            built["identity"] == self.kernel_identity(),
            "Kernel inputs changed after build",
        )
        require(
        ...
        shutil.copy2(
            self.work / "bazel-bin/common/kernel_aarch64_config/out_dir/.config",
            dest / "kernel.config",
        )
        ...
        if review := report.get("vpnhide_review"):
            with zipfile.ZipFile(
                dest / "vpnhide-review.zip", "w", compression=zipfile.ZIP_DEFLATED
            ) as archive:
                for path in sorted(Path(review).rglob("*")):
                    if path.is_file():
                        archive.write(path, path.relative_to(review))
        with zipfile.ZipFile(
            dest / "husky-kernel.zip", "w", compression=zipfile.ZIP_DEFLATED
        ) as archive:
